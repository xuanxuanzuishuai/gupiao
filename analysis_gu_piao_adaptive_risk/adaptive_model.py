...
作用:
    对风险覆盖分支做滚动回测，评估不同风险标签在多个持有周期下的
    收益、胜率和回撤表现，并据此生成动态风控配置和 Markdown 报告。
    它服务于 overlay，但不直接承担候选股票打分。

流程:
    先复用 overlay 的历史读取和市场分层能力，准备带远期收益的样本；
    再为每个风险分支统计不同持有周期表现和证据强弱；
    然后校准 block / downgrade / observe 等动作和风险分；
    最终返回 adaptive_config，供 overlay 刷新风险覆盖表时使用。
"""
...
from . import overlay as risk_overlay
import func
